<template>
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','name_of_manager') || $check_field('add','name_of_manager') || $check_field('set','name_of_manager')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="经理姓名" prop="name_of_manager">
					<el-input id="name_of_manager" v-model="form_sub['name_of_manager']" placeholder="请输入经理姓名"
							  v-if="user_group === '管理员' || (form_sub['division_manager_id'] && $check_field('set','name_of_manager')) || (!form_sub['division_manager_id'] && $check_field('add','name_of_manager'))"></el-input>
					<div v-else-if="$check_field('get','name_of_manager')">{{form_sub['name_of_manager']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['division_manager_id'] && $check_field('set','gender')) || (!form_sub['division_manager_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','age') || $check_field('add','age') || $check_field('set','age')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="年龄" prop="age">
					<el-input id="age" v-model="form_sub['age']" placeholder="请输入年龄"
							  v-if="user_group === '管理员' || (form_sub['division_manager_id'] && $check_field('set','age')) || (!form_sub['division_manager_id'] && $check_field('add','age'))"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
					"name_of_manager_isDisabled": false,
					"gender_isDisabled": false,
					"age_isDisabled": false,
				},

				form_sub: {
					"name_of_manager":'', // 经理姓名
					"gender":'', // 性别
					"age":'', // 年龄
				    "user_id": 0,
					"division_manager_id": 0 // ID
				},
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["division_manager_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?division_manager_id=" + form_sub["division_manager_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/division_manager/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/division_manager/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/division_manager/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/division_manager/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/division_manager/view','get');
